...
// Continuous acquisition; the host segments speech at silence or 30 seconds.
...
void startRecording(bool ble) {
  ...
  if (recording || stopPending) return;
...
void loop() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (raw != b.stable && now - b.changed >= 30) {
      b.stable = raw;
      if (!raw) {
        if (b.clicks > 0 && (int32_t)(now - b.clickDeadline) >= 0) {
          emitButton(i, b.clicks == 1 ? "single" : "double");
          b.clicks = 0;
        }
        b.pressed = now;
        ...
        if (b.clicks >= 3) { b.clicks = 0; emitButton(i, "triple"); }
      }
    }
    if (b.stable && b.clicks > 0 && (int32_t)(now - b.clickDeadline) >= 0) {
      emitButton(i, b.clicks == 1 ? "single" : "double");
      b.clicks = 0;
    }
  }
